...
float *gen_arr(int num_procs, char *proc_name, int local_rank, float one_prob, float minus_one_prob);
void createTwoDimMeshTopo(MPI_Comm *comm_new, int *local_rank, int *num_procs);
void createRingTopo(MPI_Comm *comm_new, int *local_rank, int *num_procs);
void createHyperTopo(MPI_Comm *comm_new, int *local_rank, int *num_procs);

float *distributeSerially(MPI_Comm *comm_new, int local_rank, int num_procs, char *proc_name, int *elem_per_node);
...
void computeDeterminant(float *A, int _n);

float *performCannonOp(MPI_Comm *comm_new, float *A, float *B, int local_rank, int num_procs);
void performStrassenOp(float *A, float *B, float *C, int _n);
void performDnsOp(MPI_Comm *comm_new, float *A, float *B, float *C, int local_rank, int num_procs);

void dnsAlignOp(float *A, float *B);

void addition(float *A, float *B, float *C, int _n);
void subtraction(float *A, float *B, float *C, int _n);

void computerMatrixPower(float *A, float *C, int _n, int _k);
void performMatrixMultSerially(float *A, float *B, float *C, int n);
void blockMatrixMultiplication(float *A, float *B, float *C, int q);


void matrixPrintOp(float *A, int nElem);
...
int main(int argc, char **argv) {
  ...
  if( type == serial || type == strassen ) {
    createRingTopo(&comm_new, &local_rank, &num_procs);
  } else if( type == cannon ) {
    createTwoDimMeshTopo(&comm_new, &local_rank, &num_procs);
  } else if( type == dns ) {
    createHyperTopo(&comm_new, &local_rank, &num_procs);
  }

  t_start = MPI_Wtime();
  if( type == serial ) {
    local_array = distributeSerially(&comm_new, local_rank, num_procs, proc_name, &elem_per_node);
    C = (float*) malloc(sizeof(float) * n * n);
    start = MPI_Wtime();
    computerMatrixPower(local_array, C, n, k);
    end = MPI_Wtime();
    dt = end - start;
    proc_time += dt;
    ...
    for( i = 2; i <= k_end; ++i) {
      C = performCannonOp(&comm_new, local_array, B, local_rank, num_procs);
    ...
    for( i = 2; i <= k_end; ++i ) {
      performStrassenOp(local_array, B, C, n);
    ...
    for( i = 2; i <= k_end; ++i) {
      ...
      performDnsOp(&comm_new, A, B, C, local_rank, num_procs);
...
int parse(int argc, char **argv) {int i, c;
int option_index = 0;
...
float *gen_arr(int num_procs, char *proc_name, int local_rank,
...
float *distributeSerially(MPI_Comm *comm_new, int local_rank, int num_procs,
  ...
  if( local_rank == 0 ) {
    local_array = gen_arr(num_procs, proc_name, local_rank, one_prob, minus_one_prob);
...
float *distribute_mesh(MPI_Comm *comm_new, int local_rank, int num_procs,
  ...
  if( local_rank == 0 ) {
    tmp_array = gen_arr(num_procs, proc_name, local_rank, one_prob, minus_one_prob);
  }
...
void computerMatrixPower(float *A, float *C, int _n, int _k) {
  ...
  for( i = 2; i <= k_end; ++i ) {
    performMatrixMultSerially(A, B, C, _n);
...
void performMatrixMultSerially(float *A, float *B, float *C, int _n) {
  bzero(C, sizeof(float) * _n * _n);
  blockMatrixMultiplication(A, B, C, _n);
}
...
void performStrassenOp(float *A, float *B, float *C, int _n) {
  ...
  addition(a11, a22, aRes, half_n);
  addition(b11, b22, bRes, half_n);
  performStrassenOp(aRes, bRes, m1, half_n); // m1 = (a11+a22) * (b11+b22)

  addition(a21, a22, aRes, half_n);
  performStrassenOp(aRes, b11, m2, half_n); // m2 = (a21+a22) * (b11)

  subtraction(b12, b22, bRes, half_n);
  performStrassenOp(a11, bRes, m3, half_n); // m3 = (a11) * (b12 - b22)

  subtraction(b21, b11, bRes, half_n);
  performStrassenOp(a22, bRes, m4, half_n); // m4 = (a22) * (b21 - b11)

  addition(a11, a12, aRes, half_n);
  performStrassenOp(aRes, b22, m5, half_n); // m5 = (a11+a12) * (b22)

  subtraction(a21, a11, aRes, half_n);
  addition(b11, b12, bRes, half_n);
  performStrassenOp(aRes, bRes, m6, half_n); // m6 = (a21-a11) * (b11+b12)

  subtraction(a12, a22, aRes, half_n);
  addition(b21, b22, bRes, half_n);
  performStrassenOp(aRes, bRes, m7, half_n); // m7 = (a12-a22) * (b21+b22)
  ...
  addition(m3, m5, c12, half_n); // c12 = p3 + p5
  addition(m2, m4, c21, half_n); // c21 = p2 + p4

  addition(m1, m4, aRes, half_n); // p1 + p4
  addition(aRes, m7, bRes, half_n); // p1 + p4 + p7
  subtraction(bRes, m5, c11, half_n); // c11 = p1 + p4 - p5 + p7

  addition(m1, m3, aRes, half_n); // p1 + p3
  addition(aRes, m6, bRes, half_n); // p1 + p3 + p6
  subtraction(bRes, m2, c22, half_n); // c22 = p1 + p3 - p2 + p6
...
float *performCannonOp(MPI_Comm *comm_new, float *A, float *B, int local_rank, int num_procs) {
  ...
  start = MPI_Wtime();
  blockMatrixMultiplication(A, B, C, n_local);
  end = MPI_Wtime();
  dt = end - start;
  proc_time += dt;
